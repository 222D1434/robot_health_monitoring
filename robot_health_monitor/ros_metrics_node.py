...
"""
ros_metrics_node.py
Nodo ROS2 que monitoriza métricas de tópicos, TFs y presencia de nodos.
"""
...
class TopicRateMonitor:
    ...
    def __init__(self, node, topic, expected_hz, tolerance, msg_type, callback_group):
        self.topic = topic
    ...
    def get_status(self) -> DiagnosticStatus:
        status = DiagnosticStatus(name=f'ros/topic_hz{self.topic}', hardware_id='ros2')
        ...
        status.values = [KeyValue(key='measured_hz', value=f'{measured_hz:.2f}')]
        return status

class RosMetricsNode(Node):
    REQUIRED_PARAMS = [
        'publish_rate', 'topics_to_monitor', 'expected_hz_list', 'tolerance_list',
        'tf_parent_frames', 'tf_child_frames', 'tf_timeouts',
        'expected_nodes', 'node_check_interval'
    ]
    ...
    def __init__(self):
        ...
        for t, hz, tol in zip(self.cfg['topics_to_monitor'], self.cfg['expected_hz_list'], self.cfg['tolerance_list']):
            m_type = self.TOPIC_TYPE_MAP.get(t)
            if m_type:
                self._topic_monitors.append(TopicRateMonitor(self, t, float(hz), float(tol), m_type, self._cb_group))
    ...
    def _timer_callback(self):
        ...
        # Check TFs
        for p, c, t in zip(self.cfg['tf_parent_frames'], self.cfg['tf_child_frames'], self.cfg['tf_timeouts']):
            status = DiagnosticStatus(name=f'ros/tf/{p}_to_{c}', hardware_id='ros2')
            try:
                if self.tf_buffer.can_transform(p, c, rclpy.time.Time(), timeout=rclpy.duration.Duration(seconds=float(t))):
        ...
        for expected in self.cfg['expected_nodes']:
            status = DiagnosticStatus(name=f"ros/node/{expected.lstrip('/')}", hardware_id='ros2')
