...
Nodo ROS2 que recopila métricas del sistema operativo.
"""
...
class SystemMetricsNode(Node):
    ...
    def _level_for_value(self, value: float, warn_key: str, error_key: str) -> int:
    ...
    def _check_cpu(self) -> DiagnosticStatus:
        ...
        status.level = self._level_for_value(cpu_pct, 'cpu_warn_threshold', 'cpu_error_threshold')
    ...
    def _check_memory(self) -> DiagnosticStatus:
        ...
        status.level = self._level_for_value(vm.percent, 'mem_warn_threshold', 'mem_error_threshold')
    ...
    def _check_disk(self) -> DiagnosticStatus:
        ...
        status.level = self._level_for_value(du.percent, 'disk_warn_threshold', 'disk_error_threshold')
    ...
    def _check_network(self) -> DiagnosticStatus:
        ...
        try:
            ...
            if not counters:
                status.level, status.message = DiagnosticStatus.ERROR, f'Iface {iface} no encontrada'
                return status
    ...
    def _check_temperature(self):
        ...
        status = DiagnosticStatus(name='system/temperature', hardware_id='rpi4')
        try:
            ...
            if t:
                cur = t[0].current
                status.level = self._level_for_value(cur, 'temp_warn_threshold', 'temp_error_threshold')
